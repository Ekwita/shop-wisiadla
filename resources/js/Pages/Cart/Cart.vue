...
const hasSelectedItems = computed(() => {
    return props.cartItems.some(item => item.selected);
});
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                                ...
                                <transition enter-active-class="transition ease-out duration-100"
                                    ...
                                    <MenuItems
                                        ...
                                        <div class="py-1">
                                            ...
                                            <button type="submit" @click="deleteSelectedItems"
                                                :class="[active ? 'bg-gray-100 text-gray-900 outline-none' : 'text-gray-700', 'block w-full px-4 py-2 text-left text-sm']">Delete
                                                selected</button>
                                            </MenuItem>
                                            <MenuItem v-slot="{ active }">
                                            <button type="submit" @click="deleteAll"
                                                :class="[active ? 'bg-gray-100 text-gray-900 outline-none' : 'text-gray-700', 'block w-full px-4 py-2 text-left text-sm']">Delete
                                                all</button>
                                            </MenuItem>
                                        </div>
                                ...
                                <form @submit.prevent="updateQuantity(item)">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <input type="number" v-model.number="item.quantity" :min="1"
                                            :max="item.product.quantity" required
                                            class="w-16 text-center border rounded-lg" @change="updateQuantity(item)" />
                ...
                <div class="lg:col-span-1 p-4 border rounded-lg shadow-sm space-y-4">
                    ...
                    <div class="flex flex-col space-y-2">
                        <button :disabled="!hasSelectedItems" :class="[
                            hasSelectedItems
                                ? 'bg-indigo-600 hover:bg-indigo-700 text-white'
                                : 'bg-gray-400 text-gray-600',
                            'px-4 py-2 rounded-lg font-semibold text-center transition-colors duration-200'
